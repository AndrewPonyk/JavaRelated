...
def _dfs(residual: list[list[int]], source: int, sink: int, visited: set[int], parent: list[int]) -> bool:
    visited.add(source)
    if source == sink:
        return True
    for vertex, capacity in enumerate(residual[source]):
        if vertex not in visited and capacity > 0:
            parent[vertex] = source
            if _dfs(residual, vertex, sink, visited, parent):
                return True
    return False
...
def ford_fulkerson(capacity: list[list[int]], source: int, sink: int) -> int:
    ...
    while True:
        ...
        if not _dfs(residual, source, sink, set(), parent):
            return max_flow
        path_flow = 10**9
        vertex = sink
        while vertex != source:
