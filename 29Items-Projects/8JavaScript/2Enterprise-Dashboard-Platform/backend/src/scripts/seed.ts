  ...
  try {
    ...
    const marketingDashboard = await prisma.dashboard.upsert({
      ...
      create: {
        ...
        isPublic: true,
        isTemplate: true, // This one is a template
        userId: demoUser.id,
        layout: JSON.stringify([
    ...
    // Create sample widgets for sales dashboard
    const widgets = [
      {
        id: 'revenue-chart',
    ...
    // Create sample dashboard analytics
    const analyticsData = [];
    const now = new Date();
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      analyticsData.push({
        ...
        avgLoadTime: Math.random() * 2 + 0.5, // 0.5-2.5 seconds
