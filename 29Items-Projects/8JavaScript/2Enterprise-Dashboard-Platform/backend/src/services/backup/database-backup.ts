...
export class DatabaseBackupManager {
  ...
  constructor(configOverride?: Partial<BackupConfig>) {
    this.config = {
      databaseUrl: process.env.DATABASE_URL || configOverride?.databaseUrl || '',
      backupDir: process.env.BACKUP_DIR || configOverride?.backupDir || './backups',
      retentionDays: parseInt(process.env.BACKUP_RETENTION_DAYS || '30') || configOverride?.retentionDays || 30,
      compression: process.env.BACKUP_COMPRESSION === 'true' || configOverride?.compression !== false,
      encryptionKey: process.env.BACKUP_ENCRYPTION_KEY || configOverride?.encryptionKey,
      remoteBucket: process.env.BACKUP_REMOTE_BUCKET || configOverride?.remoteBucket,
      slackWebhook: process.env.BACKUP_SLACK_WEBHOOK || configOverride?.slackWebhook,
      maxBackupSize: parseInt(process.env.BACKUP_MAX_SIZE_MB || '1000') || configOverride?.maxBackupSize || 1000,
      backupTimeout: parseInt(process.env.BACKUP_TIMEOUT_SECONDS || '3600') || configOverride?.backupTimeout || 3600,
    };
