...
router.get('/stats', authenticate, authorize('ADMIN', 'SUPER_ADMIN'), asyncHandler(async (req, res) => {
    try {
        ...
        const now = new Date();
        const last24Hours = backupStats.filter(backup => new Date(backup.timestamp) > new Date(now.getTime() - 24 * 60 * 60 * 1000)).length;
        const last7Days = backupStats.filter(backup => new Date(backup.timestamp) > new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000)).length;
