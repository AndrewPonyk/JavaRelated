...
public class ReactiveWebClientApplication {
	...
	public WebClient clientBuilder() {
		return WebClient.builder().baseUrl("http://localhost:8080/Spring_Reactive_war_exploded/")
	...
	private void get(WebClient webClient) {
		...
		Mono<User> result = webClient.get().uri("/{id}", 2).accept(MediaType.APPLICATION_JSON).retrieve()
		...
		Mono<User> monoResult = webClient.get().uri("/{id}", 2).accept(MediaType.APPLICATION_JSON).retrieve()
		...
		monoResult = (Mono<User>) webClient.get().uri("/{id}", 2).accept(MediaType.APPLICATION_JSON).exchange()
				.flatMap(response -> response.bodyToMono(User.class));

		Mono<ResponseEntity<User>> resultResponseEntity = (Mono<ResponseEntity<User>>) webClient.get()
				.uri("/{id}", 2).accept(MediaType.APPLICATION_JSON).exchange()
